...
def s_tu_bouche_à_oreille_bouche_1 (s, m):
    return m.group(0).replace(" ", "-")
def c_tu_celui_celle_là_ci_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return not (m.group(0).endswith("si") and morph(dDA, nextword1(s, m.end()), ":[AW]", False))
...
def s_ocr_mot_1 (s, m):
    return m.group(0).replace("l", "t").replace("L", "T")+"|"+m.group(0).replace("l", "i").replace("L", "I")
...
def c_conf_à_le_la_les_leur_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(1), m.group(1)), ":", ":[GNAWMBYŴ]")
...
def c_conf_davantage1_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(1), m.group(1)), ":V", ":Q|>(?:profiter|bénéficier|nombre) ") and not morph(dDA, nextword1(s, m.end()), ">(?:financi[eè]re?|pécuni(?:er|aire)|sociaux)s? ", False, False)
def c_conf_davantage2_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return not morph(dDA, (m.start(1), m.group(1)), ">(?:profiter|bénéficier) ", False) and not morph(dDA, nextword1(s, m.end()), ">(?:financi[eè]re?|pécuni(?:er|aire)|sociaux)s? ", False, False)
...
def c_conf_qu_elle_verbe_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return m.group(2).islower() and (morphex(dDA, (m.start(2), m.group(2)), ":V|>(?:ne?|me?|te?|se?|[nv]ous|l(?:e|a|es|ui|leur|)|en|y) ", ":[NA].*:[fe]|>(?:plus|moins)") or m.group(2) == "t" or m.group(2) == "s") and not (morph(dDA, (m.start(2), m.group(2)), ">(?:pouvoir|devoir|en)", False) and morph(dDA, nextword1(s, m.end()), ":V0e", False))
def c_conf_qu_elle_verbe_2 (s, sx, m, dDA, sCountry, bCondMemo):
    return m.group(1).endswith("e") and not morph(dDA, (m.start(2), m.group(2)), ":V0e", False) and not (morph(dDA, (m.start(2), m.group(2)), ":V0a", False) and look(s[m.end():], "^ +été "))
def c_conf_qu_elle_verbe_3 (s, sx, m, dDA, sCountry, bCondMemo):
    return not bCondMemo and m.group(1).endswith("s") and not morph(dDA, (m.start(2), m.group(2)), ":V0e", False)  and not (morph(dDA, (m.start(2), m.group(2)), ":V0a", False) and look(s[m.end():], "^ +été "))
...
def c_loc_tueur_à_gages_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return m.group(2) == "a"
def c_loc_vache_à_lait_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return m.group(2) == "a"
...
def c_conf_c_est_était_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(3), m.group(3)), ":[AM]", ":[QGW]")
...
def s_infi_de_pronom_le_les_la_1 (s, m):
    return suggVerbInfi(m.group(1))
def c_infi_faire_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morph(dDA, (m.start(1), m.group(1)), ">faire ", False) and not look(s[:m.start()], r"(?i)\b(?:en|[mtsldc]es?|[nv]ous|un) +$") and morphex(dDA, (m.start(2), m.group(2)), ":V", ":M") and not (re.search("(?i)^fait$", m.group(1)) and m.group(2).endswith("é")) and not (re.search("(?i)^faits$", m.group(1)) and m.group(2).endswith("és"))
def s_infi_faire_1 (s, m):
    return suggVerbInfi(m.group(2))
def c_infi_vouloir_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morph(dDA, (m.start(1), m.group(1)), ">vouloir ", False) and not look(s[:m.start()], r"(?i)\b(?:[mtsldc]es?|[nv]ous|un) +$") and morphex(dDA, (m.start(2), m.group(2)), ":V", ":M") and not (re.search("(?i)^vouloir$", m.group(1)) and m.group(2).endswith("é")) and not (re.search("(?i)^vouloirs$", m.group(1)) and m.group(2).endswith("és"))
def s_infi_vouloir_1 (s, m):
    return suggVerbInfi(m.group(2))
...
def c_ppas_ça_verbe_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return (morph(dDA, (m.start(2), m.group(2)), ">(?:être|sembler|devenir|re(?:ster|devenir)|para[îi]tre) ", False) or m.group(2).endswith(" été")) and ( morphex(dDA, (m.start(3), m.group(3)), ":[NAQ].*:p", ":[GWYsi]") or ( morphex(dDA, (m.start(3), m.group(3)), ":[AQ].*:f", ":[GWYme]") and not morph(dDA, nextword1(s, m.end()), ":N.*:f", False, False) ) ) and not morph(dDA, prevword1(s, m.start()), ":(?:R|V...t)|>de ", False, False)
...
def c_imp_laisser_le_la_les_infi_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morph(dDA, (m.start(2), m.group(2)), ">laisser .*:E", False) and morphex(dDA, (m.start(3), m.group(3)), ":(?:Y|X|Oo)", ":[NAB]")
...
def c_conj_nom_propre_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return not look(s[:m.start()], r"\b(?:et |ou |[dD][eu] |ni |[dD]e l’) *$") and morph(dDA, (m.start(1), m.group(1)), ":M", False) and morphex(dDA, (m.start(2), m.group(2)), ":[123][sp]", ":(?:G|3s|3p!|P|M|[AQ].*:[si]|N.*:m:s)") and not morph(dDA, prevword1(s, m.start()), ":[VRD]", False, False) and not look(s[:m.start()], r"([A-ZÉÈ][\w-]+), +([A-ZÉÈ][\w-]+), +$") and not (morph(dDA, (m.start(2), m.group(2)), ":3p", False) and prevword1(s, m.start()))
...
def s_conj_que_où_comment_verbe_sujet_sing_1 (s, m):
    return suggVerb(m.group(1), ":3s")
def c_conj_lxquel_verbe_sujet_sing_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(1), m.group(1)), ":(?:[12]s|3p)", ":(?:3s|G|W|3p!)") and not look(s[m.end():], "^ +(?:et|ou) (?:l(?:es? |a |’|eurs? )|[mts](?:a|on|es) |ce(?:tte|ts|) |[nv]o(?:s|tre) |d(?:u|es) )") and morph(dDA, prevword1(s, m.start()), ":R", False, False)
def s_conj_lxquel_verbe_sujet_sing_1 (s, m):
    return suggVerb(m.group(1), ":3s")
...
def s_conj_que_où_comment_verbe_sujet_pluriel_1 (s, m):
    return suggVerb(m.group(1), ":3p")
def c_conj_lxquel_verbe_sujet_pluriel_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(1), m.group(1)), ":[123]s", ":(?:3p|G|W)") and morph(dDA, prevword1(s, m.start()), ":R", False, False)
def s_conj_lxquel_verbe_sujet_pluriel_1 (s, m):
    return suggVerb(m.group(1), ":3p")
def c_conj_que_où_comment_verbe_sujet_1 (s, sx, m, dDA, sCountry, bCondMemo):
    return morphex(dDA, (m.start(1), m.group(1)), ":[12][sp]", ":(?:G|W|3[sp]|Y|P|Q|N|A|M)")
